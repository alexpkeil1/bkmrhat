...
```{r single vs multi-chains, results='markup', cache=TRUE}

# enable parallel processing
future::plan(strategy = future::multiprocess)

# single run of 4000 observations
set.seed(111)
system.time(kmfit <- suppressMessages(kmbayes(y = y, Z = Z, X = X, iter = 4000, verbose = FALSE, varsel = FALSE)))


# 4 runs of 1000 observations
set.seed(111)
...
Gelman's r-hat diagnostic gives an interpretable diagnostic: the expected reduction in the standard error of the posterior means if you could run the chains to an infinite size. These give some idea about when is a fine idea to start sampling. There are rules of thumb about using r-hat to stop sampling that are available from several authors (for example you can consult the help files for `rstan` and `coda`).
...
mean.difference <- OverallRiskSummaries(fit = fitkmccomb, y = y, Z = Z, X = X, 
                                      qs = seq(0.25, 0.75, by = 0.05), 
                                      q.fixed = 0.5, method = "exact")
mean.difference

ggplot(mean.difference, aes(quantile, est, ymin = est - 1.96*sd, ymax = est + 1.96*sd)) + 
  geom_pointrange() + 
  theme_classic()
```
...
set.seed(111)
system.time(kmfitbma.list <- suppressMessages(kmbayes_parallel(nchains=4, y = y, Z = Z, X = NULL, iter = 1000, verbose = FALSE, varsel = TRUE)))

kmbayes_diag(kmfitbma.list)
...
mean.difference2 <- OverallRiskSummaries(fit = kmfitbma.comb, y = y, Z = Z, X =NULL,                                       qs = seq(0.25, 0.75, by = 0.05), 
                                      q.fixed = 0.5, method = "exact")
mean.difference2

ggplot(mean.difference2, aes(quantile, est, ymin = est - 1.96*sd, ymax = est + 1.96*sd)) + 
  geom_pointrange() + 
  theme_classic()
