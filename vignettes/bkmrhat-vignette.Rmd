...
1. Fit a BKMR model for a single chain using the `kmbayes` function from `bkmr`, or use multiple parallel chains `kmbayes_parallel` from `bkmrhat`
...
The diagnostics from the rstan package come from the `monitor` function (see the help files for that function in the rstan package)
...
These results parallel the previous section and are given here without comment, other than to note that no fixed effects (X variables) are included, and that it is useful to check the posterior inclusion probabilities to ensure they are similar across chains. There are several functions ported directly from the `bkmr` package (e.g. `ExtractPIPs` -> `ExtractPIPs_parallel`) that can be used on parallel chains. Here, we check the r-hat diagnostic and see that it is higher than recommended (rstan recommends < 1.05, coda recommends < 1.01 for upper CI) and then increase iterations accordingly - it is common with Bayesian variable selection methods that more iterations are needed to achieve convergence. 
...
bmadiag = kmbayes_diag(kmfitbma.list, print=TRUE)
# r-hat seems too low, so run again with more samples: note this is common with variable selection

# first, let's visualize posterior exclusion probability of each chain in a set of chains that don't seem to have converged
pips = ExtractPIPs_parallel(kmfitbma.list)
nvars = length(unique(pips$variable))
nchains = length(unique(pips$chain))
pc = palette.colors(alpha=0.9)
...
with(pips, {
  ...
  for(cch in seq_len(nchains)){
   points(as.factor(variable[chain==cch]), PIP[chain==cch], col=pc[cch], pch=19) 
  }
  axis(1)
  axis(2)
  box(bty='l')
  legend("bottomleft", col=1:nchains, pch=19, legend=paste("chain", 1:nchains), bty="o", ncol = 2)
    title("PIPs for samples with poor convergence",
       ylab= "PIP", xlab = "Variable")
})

# now run with more samples
set.seed(111)
system.time(kmfitbma.list <- suppressWarnings(kmbayes_parallel(nchains=4, y = y, Z = Z, X = X, iter = 5000, verbose = FALSE, varsel = TRUE)))

bmadiag = kmbayes_diag(kmfitbma.list, print=TRUE)
# now check r-hat with coda package
kmfitbmacoda = as.mcmc.list(kmfitbma.list)
gelman.diag(kmfitbmacoda)
...
with(pips, {
  ...
  box(bty='l')
  legend("bottomleft", col=1:nchains, pch=19, legend=paste("chain", 1:nchains), bty="o", ncol = 2)
  title("PIPs for samples with better evidence for convergence",
       ylab= "PIP", xlab = "Variable")
})
...
ExtractPIPs(kmfitbma.comb) # posterior inclusion probabilities averaged over chains
...
with(meandifference_par, {
  ...
  for(cch in seq_len(nchains)){
    ...
    jit = runif(1, -width/2.5, width/2.5)
   points(jit+quantile[chain==cch], est[chain==cch], pch=19, col=pc[cch]) 
  ...
  legend("bottom", col=1:nchains, pch=19, lty=1, legend=paste("chain", 1:nchains), bty="n")
   title("Overall summaries, model with poor convergence",
...
with(regfuns_par[regfuns_par$variable=="z1",], {
  ...
  plot.window(ylim=c(min(est - 1.96*se), max(est + 1.96*se)), 
              xlim=c(min(z), max(z)))
  pc = palette.colors(alpha=1)
  pc2 = palette.colors(alpha=0.1)
  for(cch in seq_len(nchains)){
  ...
  box(bty='l')
  legend("bottom", col=1:nchains, pch=19, lty=1, legend=paste("chain", 1:nchains), bty="n")
   title("Single variable summary, model with poor convergence",
       ylab= "Predicted Y", xlab = "Z")
})

# now repeat with more iterations
set.seed(111)
system.time(kmfitbma.list <- suppressWarnings(kmbayes_parallel(nchains=4, y = y, Z = Z, X = X, iter = 5000, verbose = FALSE, varsel = TRUE)))
...
with(meandifference_par, {
  ...
  box(bty='l')
  legend("bottom", col=1:nchains, pch=19, lty=1, legend=paste("chain", 1:nchains), bty="n")
   title("Overall summaries, model with better evidence for convergence",
       ylab= "Mean difference", xlab = "Joint quantile")
})
...
with(regfuns_par[regfuns_par$variable=="z1",], {
  ...
  box(bty='l')
  legend("bottom", col=1:nchains, pch=19, lty=1, legend=paste("chain", 1:nchains), bty="n")
   title("Single variable summary, model with better evidence for convergence")
})

```

### Example 5b: Parallel posterior summaries as diagnostics
BKMR can also output individual level predictions of the outcome at observed or new levels of exposure
```{r post diagnostics 2, results='markup', fig.show='hold', fig.height=5, fig.width=7.5, cache=FALSE}

set.seed(111)
system.time(kmfitbmah.list <- suppressWarnings(kmbayes_parallel(nchains=4, y = y, Z = Z, X = X, iter = 1000, verbose = FALSE, varsel = TRUE, est.h =TRUE)))

postpreds = SamplePred_parallel(kmfitbma.list)
nchains = length(unique(postpreds$chain))

with(postpreds, {
  var = z50
  dens = tapply(var, chain, function(x) data.frame(x=density(x)$x, y=density(x)$y))
  alldens = do.call("rbind", dens)
  alldens$chain = rep(seq_len(nchains), each=nrow(alldens)/nchains)
  plot.new()
  plot.window(ylim=c(min(alldens$y), max(alldens$y)), 
              c(min(alldens$x), max(alldens$x)))
  pc = palette.colors(alpha=0.1)
  for(cch in seq_len(nchains)){
   polygon(alldens$x, alldens$y, pch=19, col=pc[cch]) 
  }
  axis(1)
  axis(2)
  box(bty='l')
  title(xlab= "Predicted Y", ylab = "Kernel posterior density")
  #legend("bottom", col=1:nchains, pch=19, lty=1, legend=paste("chain", 1:nchains), bty="n")
})
```

### Example 6: Probit model
BKMR handles binary outcomes via a probit model, which is implemented via data augmentation. This example shows that the probit model will often need more samples for convergence, and posterior summaries will often be extremely variable in settings where a continuous outcome might produce reasonable posterior inference in the same amount of samples.
```{r probit, results='markup', fig.show='hold', fig.height=5, fig.width=7.5, cache=FALSE}
set.seed(111)
dat <- bkmr::SimData(n = 50, M = 5, ind=1:3, Zgen="realistic", family = "binomial")
y <- dat$y
Z <- dat$Z
X <- cbind(dat$X, rnorm(50))
head(cbind(y,Z,X))
...
set.seed(111)
system.time(kmfitbmabin.list <- suppressWarnings(kmbayes_parallel(nchains=4, y = y, Z = Z, X = X, iter = 1000, verbose = FALSE, varsel = TRUE, family = "binomial")))


bindiag = kmbayes_diag(kmfitbmabin.list)

kmfitbmabincoda = as.mcmc.list(kmfitbmabin.list)
gelman.diag(kmfitbmabincoda, multivariate = FALSE)


meandifference_par = OverallRiskSummaries_parallel(kmfitbmabin.list, y = y, Z = Z, X = X ,qs = seq(0.25, 0.75, by = 0.05), q.fixed = 0.5, method = "exact")
...
with(meandifference_par, {
  ...
  box(bty='l')
  legend("bottom", col=1:nchains, pch=19, lty=1, legend=paste("chain", 1:nchains), bty="n")
   title("Overall summaries, probit model (poor convergence)",
       ylab= "Mean difference", xlab = "Joint quantile")
})


# combine multiple chains into a single chain
kmfitbmabincomb = comb_bkmrfits(kmfitbmabin.list)


# For example:
summary(kmfitbmabincomb)

meandifferencebin = suppressWarnings(OverallRiskSummaries(kmfitbmabincomb, y = y, Z = Z, X = X ,qs = seq(0.25, 0.75, by = 0.05), q.fixed = 0.5, method = "exact"))
